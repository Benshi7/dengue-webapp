...
export default function Admin () {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/dengue2')
        if (!response.ok) {
        ...
        const etario = [...new Set(jsonData.map(item => item.grupo_etario_id))]

        const tipoEvento = [
          ...new Set(jsonData.map(item => item.tipo_evento_id))
        ]

        const tipoAnio = [...new Set(jsonData.map(item => item.anio_id))]
  ...
  const filteredData = data.filter(
    item =>
      item.provincia_residencia_id == provValue &&
      item.departamento_residencia == deptValue
  )
  ...
  const agregar = () => {
    const provinciasOpciones = prov
      .map(pro => `<option value="${pro.id}">${pro.name}</option>`)
      .join('')
    const grupoEtarioOpciones = edades
      .map(
        edad =>
          `<option value="${edad}">${
            grupEtario.find(p => p.id == edad)?.name
          }</option>`
      )
      .join('')
    ...
    `,
      ...
      if (result.isConfirmed) {
        const { departamento, provincia, grupoEtario, cantidad, evento, anio } =
          result.value

        // Llamada a la API usando fetch
        fetch(
          `http://localhost:5000/api/agregar_caso/${departamento}/${provincia}/${grupoEtario}/${cantidad}/${evento}/${anio}`,
          {
            method: 'POST',
            headers: {
  ...
  const modificar = async (id, dep, provi, etario, cant, event, ano) => {
    const provinciasOpciones = prov
      .map(
        pro =>
          `<option value="${pro.id}" ${pro.id === provi ? 'selected' : ''}>${
            pro.name
          }</option>`
      )
      .join('')
    ...
    const grupoEtarioOpciones = edades
      .map(
        edad =>
          `<option value="${edad}" ${edad[0] == etario ? 'selected' : ''}>${
            grupEtario.find(p => p.id == edad)?.name
          }</option>`
      )
      .join('')
    ...
    const totalAnio = anio
      .map(
        año =>
          `<option value="${año}" ${año[0] == ano ? 'selected' : ''}>${
            anios.find(p => p.id == ano)?.name
          }</option>`
      )
      .join('')
    ...
    `,
      ...
      if (result.isConfirmed) {
        ...
        const response = fetch(
          `http://localhost:5000/api/dengue/actualizar/${id}`,
          {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json'
  ...
  return (
    <div>
      <div className='flex justify-between lg:mx-52 sm:mx-10 '>
        <div className='flex gap-5'>
          ...
          >
            ...
            {dep.map(dept =>
              dept.provincia_residencia_id == provValue ? (
      ...
      <div>
        <div className='overflow-x-auto'>
          <table className='table w-full border-collapse mt-5'>
            <thead>
              <tr className='lg:grid lg:grid-cols-7 hidden '>
                <th>Año</th>
                <th>Provincia</th>
                <th>Departamento</th>
                <th>Grupo Etario</th>
            ...
            <tbody>
              {currentItems.map((item, index) => (
                <tr
                  key={item.id}
                  className={`grid lg:grid-cols-7 grid-cols-6 gap-2 ${
                    index % 2 === 0 ? 'bg-blue-100' : 'bg-green-100'
                  } hover:bg-gray-400 `}
                >
                  <td className=''>
                    {anios.find(p => p.id === item.anio_id)?.name ||
                      'Año desconocido'}
                  </td>
                  <td className='p-2 '>
                    {prov.find(p => p.id === item.provincia_residencia_id)
            ...
            if (page >= startPage && page <= endPage) {
              return (
                <button
                  ...
                  className={` ${
                    currentPage === page
                      ? 'active text-2xl text-gray-900 bg-green-300 rounded-full'
                      : 'text-gray-600'
                  }`}
                >
                  {page}
